...
OpTable to extract and parse information, can be viewed another method for parsing data. Consider OpTable as converting the XML object into a customized dictionary-like object

ConfigTable to provision Junos config, can be viewed another method for config provisioning. Consider ConfigTable as setting the attribute of a dictionary object, and then call its function to commit the config
...
This section describes the `RouteTable` OpTable, it is defined in `op/routes.yml` file
...
To use the OpTable, import the dictionary name defined in the YAML file that has the `rpc` key, the name usually ends with 'Table'
...
Next, create the Table instance, same as the Config instance, two syntax are available - provide the `Device` object as the argument in the creation statement, or use the device `bind()` function to associate the Table with the Device

<pre>
from jnpr.junos import Device
dev = Device('vqfx1', user='root', password='VR-netlab9').open()
...
Run the `get()` function, what PyEZ does here is to run the RPC defined in the `rpc` key in YAML file, it is `<get-route-information>` here

The `get()` function accepts the same arguments as `dev.rpc.get_route_information()`, here passing in the `table='inet.0'` to specify the default route table, it is same as the CLI command `show route table inet.0`
...
All the available keys are defined in the `routes.yml` YAML file

To display all the routes information, use a for loop to iterate over the Tables object
...
To explain the relationship between the YAML and XML, put them side by side as below. Those non-related XML element tags are deleted for brevity
...
Also, the xpath of `age` is `rt-entry/age/@seconds`, `@` means getting the attribute inside the tag, it tries to fetch the attribute `seconds` inside the tag `<age seconds="4543">`, and the result is 89356 here
...
For subsequent variable assignment, it is defined under `fields_<group_name>`, it is `fields_entry` here, and all the xpath defined under the `fields_entry` will be prepended by `rt-etnry/`
...
```
<button type="button" class="btn btn-primary btn-sm" onclick="runSnippetInTab('vqfx1', 29)">Run this snippet</button>
