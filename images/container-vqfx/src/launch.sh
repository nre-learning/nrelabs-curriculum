...
if [ -e /var/run/docker.sock ]; then
  ...
  echo "no access to /var/run/docker.sock"
fi
...
if [ -f /u/$CONFIG ]; then
  ...
  echo "apply-groups openjnpr-container-vqfx;" > /tmp/$CONFIG
fi
...
for eth in $ethlist; do
  ...
  echo "groups { openjnpr-container-vqfx { interfaces { xe-0/0/$index { description "xe-0/0/${index}_$eth"; mac $mymac; mtu $mtu; unit 0; }}}}" >> /tmp/$CONFIG
  index=$(($index +1))
done

# create em1 used to connect vQFX RE VM and PFE cosim
ip tuntap add dev em1 mode tap
ip link set dev em1 mtu 9500
...
eval exec qemu-system-x86_64 -M pc --enable-kvm -cpu host  -smp 1 -m $VCPMEM \
  ...
  -netdev type=tap,id=tc1,ifname=em1,script=no,downscript=no -device virtio-net-pci,netdev=tc1,host_mtu=9500 \
